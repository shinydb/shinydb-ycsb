...
/// Document structure for workload F
...
/// Simulates a user database - read profile, update it
pub const WorkloadF = struct {
    ...
    pub fn init(allocator: Allocator, client: *ShinyDbClient, space_name: []const u8, store_name: []const u8, config: Config) !WorkloadF {
        ...
        // Zipfian distribution for hotspot access
        const key_dist = distributions.createDistribution(.zipfian, random, 0, config.record_count);
    ...
    fn executeReadModifyWrite(self: *WorkloadF) !bool {
        ...
        // Read phase
        var read_query = Query.init(self.client);
        defer read_query.deinit();

        _ = read_query.space(self.space_name)
            .store(self.store_name)
            .readById(doc_key)
            .limit(1);

        var read_result = read_query.run();
        if (read_result) |*response| {
            defer response.deinit();
        } else |err| {
            std.debug.print("RMW Read error for key {}: {}\n", .{ doc_key, err });
            if (err == error.DocumentNotFound) return false;
            return err;
        }

        // Modify phase (generate new document)
        const doc = WorkloadDoc{
            .id = key_idx,
            .field0 = self.field_data,
            .field1 = self.field_data,
        ...
        // Write phase
        var update_query = Query.init(self.client);
        defer update_query.deinit();

        _ = try update_query.space(self.space_name)
            .store(self.store_name)
            .readById(doc_key)
            .update(doc);

        var update_result = update_query.run();
        if (update_result) |*response| {
            defer response.deinit();
            return true;
        } else |err| {
            std.debug.print("RMW Update error for key {}: {}\n", .{ doc_key, err });
            if (err == error.DocumentNotFound) return false;
            return err;
        }
    }
