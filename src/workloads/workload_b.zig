...
/// Document structure for workload B
...
/// Simulates a photo tagging workload - read heavy with occasional updates
pub const WorkloadB = struct {
    ...
    pub fn init(allocator: Allocator, io: Io, client: *ShinyDbClient, space_name: []const u8, store_name: []const u8, config: Config) !WorkloadB {
    ...
    pub fn load(self: *WorkloadB) !void {
        ...
        while (inserted < self.config.record_count) : (inserted += 1) {
            ...
            if (insert_result) |*response| {
                ...
                // Parse document key from JSON response
