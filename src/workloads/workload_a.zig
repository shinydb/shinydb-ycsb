...
fn parseKeyFromJson(json: []const u8) !u128 {
    // Simple JSON parsing - find "key":"..." pattern
...
/// Document structure for workload A
...
/// Simulates a session store recording recent actions
pub const WorkloadA = struct {
    ...
    pub fn init(allocator: Allocator, io: Io, client: *ShinyDbClient, space_name: []const u8, store_name: []const u8, config: Config) !WorkloadA {
        // Allocate PRNG on heap so its address doesn't change
        ...
        // Create Zipfian distribution for key access (80/20 rule)
        ...
        // Create operation chooser for 50/50 read/update mix
    ...
    /// Phase 1: Load data - insert initial record set
    pub fn load(self: *WorkloadA) !void {
        std.debug.print("\n=== Workload A: Loading Phase ===\n", .{});

        // Drop existing space/store if they exist (ignore errors)
        self.client.drop(proto.DocType.Store, self.store_ns) catch {};
        self.client.drop(proto.DocType.Space, self.space_name) catch {};
        ...
        while (inserted < self.config.record_count) : (inserted += 1) {
            ...
            if (insert_result) |*response| {
                ...
                // Parse document key from JSON response: {"key":"<hex>"}
    ...
    /// Phase 2: Run workload - execute mixed operations
    pub fn run(self: *WorkloadA) !void {
        ...
        while (completed < self.config.operation_count) : (completed += 1) {
            ...
            const result = switch (op_type) {
                ...
                else => unreachable, // Workload A only has reads and updates
            };
